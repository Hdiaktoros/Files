...
function loadCustomCaptcha($height = 46, $width = '300px', $bgcolor = '#003', $textcolor = '#abc')
{
    ...
    $ret .= '<div style="height: ' . $height . 'px; line-height: ' . $height . 'px; width:' . $width . '; text-align: center; background-color: ' . $bgcolor . '; color: ' . $textcolor . '; font-size: ' . ($height - 20) . 'px; font-weight: bold; letter-spacing: 20px; font-family: \'Henny Penny\', cursive;  -webkit-user-select: none; -moz-user-select: none;-ms-user-select: none;user-select: none;  display: flex; justify-content: center; width:100%;">';
...
function imagePath()
{
    ...
    $data['advertise'] = [
        'path' => 'assets/images/front-image'
    ];
    $data['product'] = [
...
function advertisements($size) {
    $ad = \App\Models\Advertise::where('add_size', $size)->where('status',1)->inRandomOrder()->first();
    if(!empty($ad)){
        return  '<a  target="_blank" href="'.$ad->url.'" class="image-item clickUp" data-id="'.$ad->id.'"><img src="'.getImage('assets/images/front-image/'.$ad->image,"$ad->add_size").'" alt="image"></a>';
    } else {
        return false;
    }
}

function advertisementOne($size) {
    $ad = \App\Models\Advertise::where('add_size', $size)->where('status',1)->inRandomOrder()->first();
    if(!empty($ad)){
        return  '<a  target="_blank" href="'.$ad->url.'" class="image-item mb-5 d-none d-md-block clickUp" data-id="'.$ad->id.'"><img src="'.getImage('assets/images/front-image/'.$ad->image,"$ad->add_size").'" alt="image" class="w-100"></a>';
    } else {
        return false;
    }
}

function advertisementTwo($size) {
    $ad = \App\Models\Advertise::where('add_size', $size)->where('status',1)->inRandomOrder()->first();
    if(!empty($ad)){
        return  '<a  target="_blank" href="'.$ad->url.'" class="image-item mt-5 d-none d-md-block clickUp" data-id="'.$ad->id.'"><img src="'.getImage('assets/images/front-image/'.$ad->image,"$ad->add_size").'" alt="image" class="w-100"></a>';
    } else {
        return false;
    }
}
