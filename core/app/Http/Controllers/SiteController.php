...
class SiteController extends Controller
{
    ...
    public function index(){
        ...
        $topRatedProducts = Product::where('status', 1)->whereHas('category', function ($query) {
            $query->where('status',1);
        })->whereHas('subcategory', function ($query) {
            $query->where('status',1);
        })->selectRaw('products.*, (avg_rating*download) as point')
        ->orderBy('point', 'desc')->limit(12)->get();

        $sections = Page::where('tempname',$this->activeTemplate)->where('slug','home')->first();
        return view($this->activeTemplate . 'home', compact('pageTitle','sections','products','topRatedProducts'));
    }
    ...
    public function productDetails($slug,$id)
    {
        ...
        $moreProducts = Product::where('status',1)->whereHas('category', function ($query) {
            $query->where('status',1);
        })->whereHas('subcategory', function ($query) {
            $query->where('status',1);
        ...
        return view($this->activeTemplate.'product-details',compact('pageTitle','product','moreProducts','ratings'));
    }
    ...
    public function products()
    {
        ...
        return view($this->activeTemplate.'search',compact('pageTitle','emptyMessage','products','emptyMessage'));
    }
    ...
    public function productSearch(Request $request)
    {
        ...
        return view($this->activeTemplate.'search',compact('pageTitle','emptyMessage','products'));
    }

    public function subcategorySearch($slug,$id)
    {
        $subcategory = SubCategory::where('status',1)->findOrFail($id);
        $pageTitle = 'Products from '.$subcategory->name;
        $emptyMessage = 'No products found';

        $products = $subcategory->products()->where('status',1)->whereHas('category', function ($query) {
            $query->where('status',1);
        })->latest()->paginate(getPaginate());
    ...
    public function categorySearch($slug,$id)
    {
        ...
        $products = $category->products()->where('status',1)->whereHas('subcategory', function ($query) {
            $query->where('status',1);
        })->latest()->paginate(getPaginate());
